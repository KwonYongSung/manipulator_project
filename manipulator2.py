...
class ArucoNode(rclpy.node.Node):
    def __init__(self):
        ...
        # project parameters
        self.declare_parameter("manipulator2_param", "false") #지금은 미사용
...
# 터틀봇의 파라미터를 go로 바꾸는 함수
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            if marker_ids == [1] or marker_ids == [2]:

                time.sleep(2)
                goal_joint_angle[4] = -0.002  # 물체의 크기에 따라 정도 정하기
                teleop_keyboard.send_tool_control_request()
                time.sleep(2)
                goal_joint_angle[3] = radians(70)
                goal_joint_angle[1] = radians(-30)
                pathtime = 3.0
                teleop_keyboard.send_goal_joint_space(pathtime)
                time.sleep(2)
                goal_joint_angle[2] = radians(35)
                goal_joint_angle[3] = radians(35)
                pathtime = 3.0
                teleop_keyboard.send_goal_joint_space(pathtime)
                time.sleep(2)
                
            if marker_ids == [1]:  #1번 마커가 인식되면 특정 위치에 놓기
                goal_joint_angle[0] = radians(60)
                goal_joint_angle[1] = radians(0)
            ...
            elif marker_ids == [2]:  #2번 마커가 인식되면 특정 위치에 놓기
                goal_joint_angle[0] = radians(-60)
                goal_joint_angle[1] = radians(0)
                goal_joint_angle[2] = 0.00
                goal_joint_angle[3] = 0.00
            ...
            goal_joint_angle[0] = radians(177)
            pathtime = 5.0
            teleop_keyboard.send_goal_joint_space(pathtime)
            time.sleep(2)

            goal_joint_angle[1] = radians(40)
            goal_joint_angle[2] = radians(40)
            goal_joint_angle[3] = radians(-20)
            pathtime = 5.0
            teleop_keyboard.send_goal_joint_space(pathtime)
            teleop_keyboard.send_tool_control_request()
...
def main():
    ...
    goal_joint_angle[1] = radians(0)
    goal_joint_angle[2] = radians(0)
    goal_joint_angle[3] = radians(0)
